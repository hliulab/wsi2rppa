...
class WholeSlideImage(object):
    ...
    def visWSI(self, vis_level=0, color=(0, 255, 0), hole_color=(0, 0, 255), annot_color=(255, 0, 0),
        ...
        :param top_left:
        :param bot_right:
        :param custom_downsample:
        :param view_slide_only:
        :param number_contours:
        :param seg_display:
        :param annot_display:
        :return: 返回一个PIL对象
        '''
    ...
    def process_contours(self, save_path, patch_level=0, patch_size=256, step_size=256, **kwargs):
        """
        处理多个轮廓的函数
        :param save_path: patch保存的路径
        :param patch_level: 下采样水平，水平越高，下采样程度越大，数据大小越小
        :param patch_size: 每个patch 的大小
        :param step_size: 每个patch搜索框的步长，当与patch size相同的时候每个patch没有交集
        :param kwargs: 若干其他参数
        :return: 返回保存好的h5文件路径
        """
        ...
        # kwargs = current_patch_params.update({'patch_level': patch_level, 'patch_size': patch_size, 'step_size':
        # step_size, 'save_path': patch_save_dir})

        print(f"process_contours函数  {save_path_hdf5}")

        print("Creating patches for: ", self.name, "...", )
        elapsed = time.time()
        n_contours = len(self.contours_tissue)  # 求轮廓个数
    ...
    def process_contour(self, cont, contour_holes, patch_level, save_path, patch_size=256, step_size=256,
        ...
        img_w, img_h = self.level_dim[0]    # 原图的大小
        if use_padding:
            stop_y = start_y + h
            stop_x = start_x + w
        else:
        ...
        for i in results:
            if i is None:
                count += 1
        print(f"共有{count}个空值")
        # 去除白色或黑色图块
        results = np.array([result for result in results if
                            result is not None and (not isBlackPatch(
                                np.array(self.wsi.read_region(result, patch_level, (patch_size, patch_size))),
                                rgbThresh=50)) and (not isWhitePatch(
                                np.array(self.wsi.read_region(result, patch_level, (patch_size, patch_size))),
                                satThresh=15))])
        # results = np.array([result for result in results if result is not None])
        # isBlackPatch(np.array(self.wsi(results,patch_level,(patch_size,patch_size))),rgbThresh=50)
        print(f"去除了{l - count - len(results)}个白色或黑色图块")
    ...
    def visHeatmap(self, scores, coords, vis_level=-1,
        ...
        patch_size = np.ceil(np.array(patch_size) * np.array(scale)).astype(int)  # 向上取整
        ...
        overlay = np.full(np.flip(region_size), 0).astype(float)    # np.flip翻转，np.full返回一个指定形状的数组
        ...
        if not blank_canvas:
            # down sample original image and use as canvas
            img = np.array(self.wsi.read_region(top_left, vis_level, region_size).convert("RGB"))
        else:
            # use blank canvas
...
if __name__ == '__main__':
    wsi = WholeSlideImage(r'D:\QQ\我的文档\990409144\FileRecv\x144045-1.tif')
    print(wsi.level_dim)
    print(wsi.level_downsamples)
    img = wsi.visWSI(0)
    # print(img)
    # img.show()
    # img = wsi.wsi.read_region((43842, 60703), 0, (256, 256))
    # print(isWhitePatch(np.array(img), 90))
    # print(img)
    # img.show()
    # wsi.wsi.properties['aperio.AppMag']
