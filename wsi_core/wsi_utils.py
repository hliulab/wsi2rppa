...
def save_hdf5(output_path, asset_dict, attr_dict=None, mode='a'):
    ...
    # litter = splited_path[-2]
    # splited_path.remove(litter)
    # output_path = "/".join(splited_path)

    # splited_path = output_path.split("/")
    # slide_name = splited_path[-1]
    # output_path = "/".join(["RESULT_DIRECTORY/patches", slide_name])
...
def sample_10(scores):
    top_k_ids = scores.argsort()[:]
    # list(range(0, len(top_k_ids), 10))
    my_list = []
    for i in list(range(0, len(top_k_ids), int(0.1*len(top_k_ids)))):
        my_list.append(top_k_ids[i])
    return my_list[:10]
...
def screen_coords(scores, coords, top_left, bot_right):
    ...
    mask = np.logical_and(np.all(coords >= top_left, axis=1), np.all(coords <= bot_right, axis=1))  # 逐元素逻辑与运算
    scores = scores[mask]
    coords = coords[mask]
...
def StitchCoords(hdf5_file_path, wsi_object, downscale=16, draw_grid=False, bg_color=(0, 0, 0), alpha=-1):
    ...
    vis_level = wsi.get_best_level_for_downsample(downscale)  # 以给定的下采样值返回一个最佳的显示图像的水平
    file = h5py.File(hdf5_file_path, 'r')
    dset = file['coords']
    coords = dset[:]  # 包含了所有patch的坐标
    w, h = wsi.level_dimensions[0]  # 原始图像的像素值，行高和列宽

    print('start stitching {}'.format(dset.attrs['name']))  # patch文件夹下h5文件的name
    print('original size: {} x {}'.format(w, h))

    w, h = wsi.level_dimensions[vis_level]  # 原始图像经各个level水平缩放后的长和宽，vis_level是自动计算的
